--CREATE PROC [dbo].[proc_CKYC_Customer217DataUploadGL] @Product VARCHAR(50),@BatchId INT = 0
--AS
BEGIN
--Check again later
...
--------Generate Batch Id 
DECLARE @BatchId INT; -- Remove later
SET @batchId = 0

DECLARE @Product VARCHAR = 'GL'
...
BEGIN
		...
		(SELECT
			...
			,ProspectNo
			,BusinessCode
			,ROW_NUMBER() OVER (PARTITION BY CUID ORDER BY CreatedOn DESC) as rn
		FROM ISOM.LOANS.PrimaryApplicant PA WITH (NOLOCK)
		LEFT OUTER JOIN dbo.StagingCustom217CustomerFlat S WITH (NOLOCK)
		ON S.SourceSystemCustomerCode = PA.CUID
		WHERE BusinessCode = 'GL'
			AND S.SourceSystemCustomerCode IS NOT NULL
...
INSERT INTO Clients.dbo.StagingCustom217CustomerFlat (
		...
		SELECT CTE.ProspectNo AS ApplicationRefNumber
			...
			,'GL' +FORMAT(GETDATE(), 'yyyyMMddHHmm') +
				RIGHT('0000000' + CONVERT(VARCHAR(10), DENSE_RANK() OVER (ORDER BY [CTE].[CUID])), 10) AS TransactionID
			...
			,533 BatchId -- change to variable later
			,'Y' IsValid
		FROM ISOM.LOANS.PrimaryApplicant PA WITH (NOLOCK)
		...
		JOIN CTE ON CTE.ProspectNo = PA.ProspectNo AND CTE.BusinessCode = PA.BusinessCode
		WHERE PA.BusinessCode = 'GL' AND CTE.rn = 1
...
------Update addresses----
		...
		SET A.PermanentCKYCAddressType = LA.AddressType
			...
			,A.PermanentAddressCity = VA.City_Description
			,A.PermanentAddressState = LA.STATE
		FROM dbo.StagingCustom217CustomerFlat A
		INNER JOIN ISOM.Loans.Addresses LA 
		ON A.ApplicationRefNumber = LA.ProspectNo
			AND LA.BusinessCode = 'GL'
			AND LA.AddressType = 'CURRENT ADDRESS AS PER KYC'
		INNER JOIN dbo.view_Addresses_refer VA
		ON A.ApplicationRefNumber=VA.ad_ProspectNo
		WHERE  A.BatchId=533 AND 
			A.ApplicationRefNumber = LA.ProspectNo
		...
		SET B.CorrespondenceAddressCountry = 'IN'
			...
			,B.CorrespondenceAddressCity =VA.City_Description
			,B.CorrespondenceAddressState = LA.STATE
		FROM dbo.StagingCustom217CustomerFlat B
		INNER JOIN ISOM.Loans.Addresses LA WITH (NOLOCK)
		ON B.ApplicationRefNumber = LA.ProspectNo
			AND LA.BusinessCode = 'GL'
			AND LA.AddressType = 'Contact Details'
		INNER JOIN dbo.view_Addresses_refer VA
		ON B.ApplicationRefNumber=VA.ad_ProspectNo
		WHERE B.BatchId=533 AND 
			B.ApplicationRefNumber = LA.ProspectNo
	...
	SET 
		...
		,B.InvalidColRemarks= CASE
									...
									WHEN PermanentAddressProof LIKE '%Aadhar%'
										...
										'Aadhar number is not present for uploaded image.'
...
SELECT 
	...
	,CTE_Image.mkrid
	,GETDATE() AS AddedOn
	,CONVERT(VARCHAR(100), 533) AS Batchid


FROM dbo.StagingCustom217CustomerFlat s217 WITH(NOLOCK)
INNER JOIN CTE_Image 
    ON CTE_Image.CUID = s217.SourceSystemCustomerCode
