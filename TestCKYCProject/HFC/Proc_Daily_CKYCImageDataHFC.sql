...
Parameters:-      
 Input:      
 Output:      
 Query to execute Procedure - EXEC Proc_CKYCImageDataHFC '20160901', '20161231', 'Y' 
...
*************************************************************************************************
*/
CREATE PROCEDURE [dbo].[Proc_Daily_CKYCImageDataHFC]
...
BEGIN
	...
	SELECT DISTINCT APP.ProspectNo
		,APP.cuid
	...
	WHERE app.ClientType = 'I'
		AND app.ApplicantType = 'APPLICANT'
	...
	FROM (
		...
		FROM (
			...
			LEFT JOIN IILHFC.dbo.tbl_TUBasicInfoAddFields TUB WITH (NOLOCK) ON CM.PROSPECTNO = tUB.PROSPECTNO
				AND TUB.APPLICANTTYPE = 'APPLICANT'
				AND TUB.ApplicantName = replace(CM.fName + CM.mNAme + CM.lName, ' ', '')
	...
	SELECT dv.srno
		...
		,DV.ImageName AS AttachmentCode
		,DV.prospectno AS AppRefNoForImages
		,POA.SourceSysytemCustomerCode AS SourceSysytemCustomerCode
		,'' AS ClientImage
		,'N' AS photouploaded
		,GETDATE() AS AddedOn
		,DV.Mkrid
	INTO #APPLICANTIMAGE
	FROM IILHFC.dbo.tbl_ICMDocumentValidate DV WITH (NOLOCK)
	INNER JOIN #CTEDOCUMENT_POA POA WITH (NOLOCK) ON POA.PROSPECTNO = DV.PROSPECTNO
		AND DV.ApplicantType = POA.ApplicantType
		AND DV.ApplicantName = POA.ApplicantName
	WHERE DV.IMAGENAME = 'APPLICANTPHOTO'
		AND DV.APPLICANTTYPE = 'APPLICANT'
	...
	SELECT 'InstaApplicant' AS PackageName
		,a.ProspectNo
		,a.Cuid
		,'' AS Reason
	...
	SELECT PROSPECTNO = APP.PROSPECTNO
		...
		,KYCEmployeeDesignation = (
			CASE 
				WHEN isnull(emp.em_Designation, '') = ''
					AND app.Mkrid LIKE '%E%'
					THEN 'Sales officer'
				ELSE COALESCE(emp.em_Designation, '')
				END
			)
		...
		,PermanentPin = CASE 
			...
			WHEN rtrim(ltrim(isnull(Iorg1.Pincode, ''))) != ''
				THEN Iorg1.Pincode
			WHEN rtrim(ltrim(isnull(Iorg.Pincode, ''))) != ''
				THEN Iorg.Pincode
			ELSE ''
			END
		...
		,PermanentAddressLine1 = SUBSTRING(CASE 
				...
				WHEN rtrim(ltrim(isnull(iorg1.add1, ''))) != ''
					THEN iorg1.add1
				WHEN rtrim(ltrim(isnull(iorg.add1, ''))) != ''
					THEN iorg.add1
				ELSE ''
		...
		,PermanentAddressLine2 = CASE 
			...
			WHEN rtrim(ltrim(isnull(iorg1.add2, ''))) != ''
				THEN iorg1.add2
			WHEN rtrim(ltrim(isnull(iorg.add2, ''))) != ''
				THEN iorg.add2
			ELSE ''
			END
		...
		,PermanentAddressLine3 = CASE 
			...
			WHEN rtrim(ltrim(isnull(iorg1.add3, ''))) != ''
				THEN iorg1.add3
			WHEN rtrim(ltrim(isnull(iorg.add3, ''))) != ''
				THEN iorg.add3
			ELSE ''
			END
		,PermanentDistrict = ''
		...
		,[APP].[ProspectCreationDate] AS CustomerStatusEffectiveDate
		,ROW_NUMBER() OVER (
	...
	--INNER JOIN #CTEDOCUMENTS_POI POI WITH (NOLOCK) ON POI.PROSPECTNO = APP.PROSPECTNO
	...
	LEFT JOIN IILHFC.dbo.state_master SM2 WITH (NOLOCK) ON iorg.StateCode = SM2.State_Code
		AND SM2.State_status = 'Y'
	LEFT JOIN IILHFC.dbo.City_Master CM2 WITH (NOLOCK) ON iorg.CityCode = CM2.City_Code
		AND (
			(
				SM2.State_Code = 'XX'
				AND 1 = 1
	...
	LEFT OUTER JOIN [UCID].[dbo].[Unique_Customers] [U] WITH (NOLOCK) ON [APP].[CUID] = [U].[CustomerID]
		AND [U].[Business] = 'HFC'
	...
	FROM (
		SELECT 'HFC' + CONVERT(VARCHAR(10), GETDATE(), 112) + convert(VARCHAR(2), Datepart(HOUR, getdate())) + convert(VARCHAR(2), Datepart(MINUTE, getdate())) + RIGHT('0000000' + CONVERT(VARCHAR(10), DENSE_RANK() OVER (
			...
			,CASE 
				...
				WHEN ISNULL([CTE].[SpouseFirstName], '') <> ''
					AND ISNULL([CTE].[FatherFirstName], '') <> ''
					THEN ISNULL([CTE].[SpouseFirstName], '')
			...
			,[CTE].[PermanentAddressProof] AS CorrespondenceAddressProof
			...
			,CustomerStatusEffectiveDate
		FROM #APPLICANT [CTE]
		--LEFT JOIN Clients..STAGINGCUSTOMERVIEWHFC S ON S.SourceSystemCustomerCode = CTE.CUID AND S.SourceSystemName = 'HFC' --AND S.UPLOADSTATUS='N'
		WHERE [CTE].rn = 1
			--S.SourceSystemCustomerCode IS NULL --[CTE].[MotherFirstName] <> '' --AND ISNULL(CTE.CorrespondenceGlobalCity,'')<>'' AND ISNULL(CTE.PermanentCity,'')<>''
			--AND 
		) AS Final

	--INSERT INTO  CKYC_RecordNotUploadReasonLog( PackageName, ProspectNo, Cuid, Reason, CreatedOn, BatchId, CreatedBy, Business)
	--SELECT 'InstaApplicant' AS PackageName, '' AS ProspectNo, a.Cuid, 'Already uploaded on Trackwizz' AS Reason
	--, GETDATE() AS CreatedOn, @BatchId AS BatchId, 'SYS' AS CreatedBy, 'HFC' AS Business
	--FROM #UploadData  A
	...
	--================================================================#ALL ATTACHMENTS==============================================================
	INSERT INTO DailyCustomerImageHFC (
		NAME
		,filepath
		,filetype
		,documenttype
	...
	FROM (
		...
		FROM (
			SELECT POA.srno
				...
				,POA.mkrid
				,ROW_NUMBER() OVER (
					PARTITION BY POA.SourceSysytemCustomerCode ORDER BY POA.SourceSysytemCustomerCode
					) AS ROWNUM
			FROM #CTEDOCUMENT_POA POA
			INNER JOIN #CustomerData217 cd ON POA.SourceSysytemCustomerCode = cd.SourceSystemCustomerCode
				AND POA.AppRefNoForImages = cd.ApplicationRefNumber
		...
		WHERE ROWNUM = 1
		--LEFT JOIN StagingCKYCAttachmentViewHFC SVF ON POA.AppRefNoForImages = SVF.AppRefnumberForImages
		--	AND POA.SourceSysytemCustomerCode = SVF.SourceSysytemCustomerCode
		...
		FROM (
			SELECT A.srno
				...
				,A.mkrid
				,ROW_NUMBER() OVER (
					PARTITION BY A.SourceSysytemCustomerCode ORDER BY A.SourceSysytemCustomerCode
					) AS ROWNUM
			FROM #APPLICANTIMAGE A
			INNER JOIN #CustomerData217 cd ON A.SourceSysytemCustomerCode = cd.SourceSystemCustomerCode
				AND A.AppRefNoForImages = cd.ApplicationRefNumber
		...
		WHERE ROWNUM = 1
			--LEFT JOIN StagingCKYCAttachmentViewHFC SVF ON A.AppRefNoForImages = SVF.AppRefnumberForImages
			--	AND A.SourceSysytemCustomerCode = SVF.SourceSysytemCustomerCode
			--WHERE SVF.SourceSysytemCustomerCode IS NULL
		) AS T
